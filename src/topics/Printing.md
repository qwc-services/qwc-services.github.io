...
## Layouts with several map items <a name="print-multiple-maps"></a>

A print layout may contain more than one map item, for instance a main map plus a locator inset or a second thematic map.

One map item follows the interactive print selection, as it always has. Every other map item is printed from the extent and the layers saved in the layout, exactly as QGIS desktop renders it. To mark a map item as such, freeze it in the QGIS layout designer, in the map item properties:

- either check `Lock layers`, so the map keeps its own layer set,
- or check `Follow map theme` and choose a theme, so the map keeps that theme's layers and styles.

A map item that is not frozen cannot follow the interactive selection, because only one item can, and QGIS Server removes from the composition any map item the print request carries no extent for. If a layout contains several unfrozen map items, the first one follows the selection and the ConfigGenerator logs a warning for each of the others, which will be missing from the printed sheet. Freezing the extra map items is therefore the way to have them printed.

Conversely, if every map item of a layout is frozen, the first one still has to follow the interactive selection, so its own extent and layers are ignored and the ConfigGenerator logs a warning. Leave the map item that is meant to follow the selection unfrozen.

Note that checking `Follow map theme` without choosing a theme leaves the map item unfrozen, because QGIS itself then renders it from the generally visible layers like any ordinary map.

Nothing changes for a layout with a single map item.

This works only for themes generated by the ConfigGenerator from the QGIS project file. The `themesConfig.{py,js}` scripts cannot support it, because GetProjectSettings exposes neither the extent saved in each map item nor whether the item is frozen.

An atlas controlled map item is always the one that follows the selection, and it may be combined with frozen map items on the same sheet.
