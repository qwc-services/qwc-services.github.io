...
* When using joins and provider feature filters in your edit layer porperties, filter expression must incluce full field path as `'schema'.'field'` in the querry. 
...
For `Autogenerated` and `Drag and Drop Designer` forms, configure the 1:N relation in QGIS Project &rarr; Properties &rarr; Relations.
