...
- `if`: Features to identify on startup, see below.
- `task`: Task to automatically activate on application start, i.e `task={"key":"LayerTree"}`, `task={"key":"Measure","mode":"Polygon"}` or `task={"key":"Editing","data":{"layer":"qwc_demo#edit_polygons","feature":3}}`. Takes precedence over the theme configuration and global configuration parameters `startupTask`.
...
### Identify features on startup

You can identify features by attribute on startup by specifying an `if` URL-parameter with a JSON-serialized string as follows:

```
if={"<layer>": <data_service_filter_expression>, ...}
```

The value is the same layer to expression mapping as the `filter` of a predefined map filter, see [Map filtering](./MapFilter.md) for the expression format. `<layer>` is the WMS name of a theme layer, optionally prefixed with the WMS name of the theme as `<theme_wms_name>#<layer>`. The prefixed form is required if multiple WMS layers in the map expose the same layer name.

Example, identifying three countries by name:

```
?t=qwc_demo&if={"countries":["name","IN",["Germany","Austria","Poland"]]}
```

Features of multiple layers can be identified at once:

```
?t=qwc_demo&if={"countries":["name","=","France"],"country_names":["name","=","France"]}
```

The matching features are queried via WMS GetFeatureInfo, listed in the identify results window and highlighted on the map, and the map is zoomed to the combined extent of the results.

Notes:

- Use `=` with a single value and `IN` with a list of values. The operator must match the value, an expression like `["name", "=", ["France", "Italy"]]` is rejected by QGIS Server.
- The queried layers must be visible and queryable when the application starts, and the theme `identifyTool` must be `Identify`.
- If a startup filter is specified via `f`, the filter of the affected layers is combined with the identify filter, so that no feature which the map filter hides is identified.
- The number of returned features per layer is limited by `feature_count` resp. `region_feature_count` of the `Identify` plugin `params` configuration.
- The theme configuration must provide the WMS name of the theme, which is the case for themes generated by the `qwc-config-generator`.
- The parameter is removed from the URL once it has been handled.
