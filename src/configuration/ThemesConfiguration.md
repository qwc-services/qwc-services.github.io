...
| `"extent": [<xmin>, <ymin>, <xmax>, <ymax>],`        | Optional, override the initial theme extent. In `mapCrs`.                                                         |
...
| `"extraPrintTemplates": ["<qpt_path>"; ...]`         | Optional, list of paths to extra print templates which should be added to the theme. See [Printing](../topics/Printing.md#layout-templates). |
...
| `"mapExtent": "<EPSG code>",`                        | Optional, the extent of the full allowed area of the map.                                             |
...
| `},`                                                 |                                                                                                       |
| `"wmsBasicAuth": "{`                                 | Optional, allows to authenticate to QGIS Server during themes.json generation. NOTE: these credentials will solely be used by `npm run themesConfig` and won't be stored in `themes.json`.|
...
```
  Note: this command is automatically invoked when starting the development server via `npm start`.
