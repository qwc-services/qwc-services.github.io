...
|`projections`                        | A list of map projections to register, in the format `{"code": "<code>", "proj": "<proj4def>", "label": "<label>", "precision": <decimals>, "format": "<decimal|dm|dms>", "addDirection": "<none|prefix|suffix|>", "swapLonLat": <true|false>}`. By default, `EPSG:3857` and `EPSG:4326` are registered. |
