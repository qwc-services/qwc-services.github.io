...
serve:
    .venv/bin/mkdocs serve -f qwc2.yml
...
# Update version and generate documentation from JSON schemas
update:
    #!/usr/bin/env bash
    set -euo pipefail
    ...
    echo "" >> src/references/index.md
    echo "qwc-qgs-cache-preseed_readme.md" >> src/references/.gitignore
    ...
    mkdir -p src/references/schemas
    for schemaUrl in \
    ...
    do
        ...
        .venv/bin/generate-schema-doc src/references/schemas/$service.json src/references/$service.md
...
build:
    ...
    if [ "{{local_build}}" == "1" ]; then
        .venv/bin/mkdocs build -f qwc2.yml
    else
